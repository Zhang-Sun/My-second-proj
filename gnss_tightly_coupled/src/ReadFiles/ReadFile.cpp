// 
// Created by wlzhang on 2022/7/4.
// 
...
namespace IGCoupled{
    ...
    void cMatchFile::InitMatchFile(tIGCOUPLEDConf &C,char sep) {
        sep_=sep;
        C_=&C;
    ...
    int cMatchFile::MatchFileAuto() {
		...
		if(C_->gnssC.ar_mode==AR_PPP_AR && (C_->gnssC.ar_prod==AR_PROD_OSB_CNES || C_->gnssC.ar_prod==AR_PROD_OSB_WUH)){} // OSB 不匹配dcb zwl 2022.4.10
        else if(!MatchCodeDcb()){
            return 0;
        }
        if(!MatchOut()){
            return 0;
        ...
        if(C_->solC.out_err_fmt){
            ...
            if((access(f,0))!=-1){
                ...
                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"ERP FILE: "<<C_->fileC.snx;
            }
            else{
    ...
    int cMatchFile::MatchProd() {
        ...
        CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"PRODUCT DIR: "<<prods_dir;

        sprintf(f,"%s%cbrdm%03d0.%02dp",prods_dir.c_str(),sep_,doy_,yy_);
        if((access(f,0))==-1){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"BRDM FILE NO FOUND: "<<f;
            return 0;
        }
        C_->fileC.brd=f;
        CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"BROADCAST EPHEMERICS FILE: "<<C_->fileC.brd;
        f[0]='\0';
        ...
        if(ion){
            ...
            if((access(f,0))==-1){
                CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"IONOSPHERE PRODUCT NO FOUND: "<<f;
                return 0;
            }
            C_->fileC.gim=f;f[0]='\0';
            CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"GIM FILE: "<<C_->fileC.gim;
        }


		if(C_->gnssC.ar_mode==AR_PPP_AR &&(C_->gnssC.ar_prod == AR_PROD_OSB_WUH || C_->gnssC.ar_prod == AR_PROD_OSB_CNES)){
            // OSB 不匹配dcb
        }
        else{
            sprintf(f,"%s%cCAS0MGXRAP_%d%03d0000_01D_01D_DCB.BSX",prods_dir.c_str(),sep_,year_,doy_);
            if(access(f,0)==-1){
                CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"CAS DCB NO FOUND: "<<f;
                return 0;
            }
            else {
                C_->fileC.cbias = f; f[0] = '\0';
                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID) << "CAS DCB FILE: " << C_->fileC.cbias;
            }
        }
        ...
        */
        if((C_->mode==MODE_PPP||C_->mode_opt==MODE_OPT_PPP)&&C_->gnssC.ar_mode==AR_PPP_AR){
            if(C_->gnssC.ar_prod==AR_PROD_FCB_SGG){
                ...
                if(access(f,0)==-1){
                    CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"FCB NO FOUND: "<<f;
                    return 0;
                }
            }

			else if(C_->gnssC.ar_prod==AR_PROD_OSB_WUH){//OSB whu 的bias
                sprintf(f,"%s%cWUM0MGXRAP_%d%03d0000_01D_01D_ABS.BIA",prods_dir.c_str(),sep_,year_,doy_);
                if(access(f,0)==-1){
                    sprintf(f,"%s%cWUM0IGSFIN_%d%03d0000_01D_01D_ABS.BIA",prods_dir.c_str(),sep_,year_,doy_);
                    if(access(f,0)==-1){
                        CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID) <<"OSB FILE NOT FOUND" <<f;
                        return 0;
                    }
                }
                C_->fileC.osb=f;f[0]='\0';
                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"OSB FILE: " <<C_->fileC.osb;
            }
            else if(C_->gnssC.ar_prod==AR_PROD_OSB_CNES){ // osb cnes
                sprintf(f,"%s%cGBM0MGXRAP_%d%03d0000_01D_30S_ABS.BIA",prods_dir.c_str(),sep_,year_,doy_);
                if(access(f,0)==-1){
                    CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID) <<"OSB FILE NOT FOUND" <<f;
                    return 0;
                }
    ...
    int cMatchFile::MatchPrec() {
        ...
        CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"PRECISE PRODUCTS DIR: "<<pres_dir;
        if(C_->gnssC.eph_opt==EPH_PRE){
            sprintf(f,"%s%c%s%d%d%s",pres_dir.c_str(),sep_,kGnssAcStr[C_->gnssC.ac_opt].c_str(),week_,wod_,".sp3");
            if((access(f,0))==-1){
				if(C_->gnssC.ar_prod==AR_PROD_OSB_WUH && C_->gnssC.ar_mode){
                    sprintf(f,"%s%cWUM0MGXRAP_%d%03d0000_01D_01M_ORB.SP3",pres_dir.c_str(),sep_,year_,doy_);
                }else if(C_->gnssC.ar_prod==AR_PROD_OSB_CNES&&C_->gnssC.ar_mode){
                    sprintf(f,"%s%cGBM0MGXFIN_%d%03d0000_01D_15M_ORB.SP3",pres_dir.c_str(),sep_,year_,doy_);
                }else{
                    sprintf(f,"%s%c%s0MGXFIN_%d%03d0000_01D_15M_ORB.SP3",pres_dir.c_str(),sep_,kGnssAcStr[C_->gnssC.ac_opt].c_str(),year_,doy_);
                }
                if((access(f,0)) == -1){
                    CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<" PRECISE ORBIT FILE NO FOUND: "<<f;
                    return 0;
                }
            }
            C_->fileC.sp3[1]=f;f[0]='\0';
            CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"ON THE DAY SP3 FILE: "<<C_->fileC.sp3[1];
			...
			if(doy_==1){
                year=year_-1;
                doy=(year%400==0 || year % 4 == 0)? 366 : 365;
            }else{
                year=year_;
                doy=doy_-1;
            }
            ...
            if((access(f,0))==-1){
				if(C_->gnssC.ar_prod==AR_PROD_OSB_WUH && C_->gnssC.ar_mode){
                    sprintf(f,"%s%cWUM0MGXRAP_%d%03d0000_01D_01M_ORB.SP3",pres_dir.c_str(),sep_,year,doy);
                }else if(C_->gnssC.ar_prod==AR_PROD_OSB_CNES&&C_->gnssC.ar_mode){
                    sprintf(f,"%s%cGBM0MGXFIN_%d%03d0000_01D_15M_ORB.SP3",pres_dir.c_str(),sep_,year,doy);
                }else{
                    sprintf(f,"%s%c%s0MGXFIN_%d%03d0000_01D_15M_ORB.SP3",pres_dir.c_str(),sep_,kGnssAcStr[C_->gnssC.ac_opt].c_str(),year,doy);
                }
                if((access(f,0)) == -1){
                    CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<" BEFORE DAY SP3 FILE NO FOUND: "<<f;
                }else{
                    C_->fileC.sp3[0]=f;f[0]='\0';
                    CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"BEFORE DAY SP3 FILE: "<<C_->fileC.sp3[0];
                }
            }
            else{
                C_->fileC.sp3[0]=f;f[0]='\0';
			...
			if(doy_==365){
                year=(year%400==0||year%4==0)?year_:year_+1;
                doy=(year%400==0 || year % 4 == 0)? 366 : 1;
            }else{
                year=year_;
                doy=doy_+1;
            }
            ...
            if((access(f,0))==-1){
                ...
                if((access(f,0)) == -1){
                    CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<" AFTER DAY SP3 FILE NO FOUND: "<<f;
                }else{
                    C_->fileC.sp3[2]=f;f[0]='\0';
                    CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"AFTER DAY SP3 FILE: "<<C_->fileC.sp3[2];
                }
            }
            else{
                C_->fileC.sp3[2]=f;f[0]='\0';
            ...
            if((access(f,0))==-1){
				if(C_->gnssC.ar_mode&&C_->gnssC.ar_prod==AR_PROD_OSB_WUH){
                    sprintf(f,"%s%cWUM0MGXRAP_%d%03d0000_01D_30S_CLK.CLK",pres_dir.c_str(),sep_,year_,doy_);
                }else if(C_->gnssC.ar_mode&&C_->gnssC.ar_prod==AR_PROD_OSB_CNES){
                    sprintf(f,"%s%cGBM0MGXFIN_%d%03d0000_01D_30S_CLK.CLK",pres_dir.c_str(),sep_,year_,doy_);
                }else{
                    sprintf(f,"%s%c%s0MGXFIN_%d%03d0000_01D_30S_CLK.CLK",pres_dir.c_str(),sep_,kGnssAcMgexStr[C_->gnssC.ac_opt].c_str(),year_,doy_);
                }
                if((access(f,0))==-1){
                    CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"PRECISE CLOCK FILE NO FOUND: "<<f;
                    return 0;
                }
            }
            C_->fileC.clk=f;f[0]='\0';
            CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"PRECISE CLOCK FILE: "<<C_->fileC.clk;

            if(C_->gnssC.ar_prod==AR_PROD_IRC_CNES){
                sprintf(f,"%s%c%s%d%d%s",pres_dir.c_str(),sep_,kGnssAcStr[1].c_str(),week_,wod_,".clk");
                if((access(f,0))==-1){
                    CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"PRECISE PSEUDORANGE SATELLITE CLOCK FILE NO FOUND FOR CNES AR: "<<f;
                    return 0;
                }
                C_->fileC.pr_clk=f;f[0]='\0';
                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"PRECISE PSEUDORANGE SATELLITE CLOCK FILE: "<<C_->fileC.pr_clk;
            }
        }
        if(C_->gnssC.tid_opt>TID_OFF){
            ...
            if((access(f,0))!=-1){
                ...
                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"ERP FILE: "<<C_->fileC.erp;
            }
            else{
                if(!C_->use_custom_dir){
                ...
                if((access(f,0))==-1){
					if(C_->gnssC.ar_mode&&C_->gnssC.ar_prod==AR_PROD_OSB_WUH){
                        sprintf(f, "%s%cWUM0MGXRAP_%d%03d0000_01D_01D_ERP.ERP",C_->data_dir.c_str(),sep_,year_,doy_);
                    }else if(C_->gnssC.ar_mode&&C_->gnssC.ar_prod==AR_PROD_OSB_CNES){
                        sprintf(f, "%s%cGBM0MGXRAP_%d%03d0000_01D_01D_ERP.ERP",C_->data_dir.c_str(),sep_,year_,doy_);
                    }else{
                        sprintf(f, "%s%c%s0MGXFIN_%d%03d0000_01D_01D_ERP.ERP",C_->data_dir.c_str(),sep_,kGnssAcMgexStr[C_->gnssC.ac_opt].c_str(),year_,doy_);
                    }

                    if((access(f,0)) == -1){
                        CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<"ERP FILE NO FOUND "<<f;
                    }
                }
                C_->fileC.erp=f;
                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"ERP FILE: "<<C_->fileC.erp;
    ...
    int cMatchFile::MatchCmn() {
        ...
        CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"COMMON PRODUCTS DIR: "<<cmn_prods;
        if(C_->gnssC.rec_ant||C_->gnssC.sat_pcv)
        {
            sprintf(f,"%s%cigs14_2097.atx",cmn_prods.c_str(),sep_);
            if((access(f,0))==-1){
                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"ATX FILE NO FOUND: "<<f;
            }
            C_->fileC.atx=f;f[0]='\0';
            CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"ATX FILE: "<<C_->fileC.atx;
        }
        if(C_->gnssC.tid_opt){
            sprintf(f,"%s%cocnload.blq",cmn_prods.c_str(),sep_);
            if((access(f,0))==-1){
                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"BLQ FILE NO FOUND: "<<f;
            }
            C_->fileC.blq=f;
            CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"BLQ FILE: "<<C_->fileC.blq;
    ...
    int cMatchFile::MatchCodeDcb() {
        ...
        if(!no_match){
            ...
            CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"CODE DCB DIR:"<<C_->fileC.cod_dcb;
        }
        return 1;
    }
    ...
    int cMatchFile::MatchOut() {
        ...
        sprintf(f,"%s%c%s_%s%c",out_dir.c_str(),sep_,kIgcoupledModeStr[C_->mode].c_str(),kIgcoupledModeOptStr[C_->mode_opt].c_str(),sep_);
        mode_dir=f;f[0]='\0';
        ...
        if(C_->solC.out_stat){
            ...
            C_->fileC.sol_stat=f;
            CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"SOLUTION FILE: "<<C_->fileC.sol;
        }
        f[0]='\0';
        return 1;
    }
    ...
    bool cReadFile::OpenFile() {
        inf_.open(file_);
        if(!inf_.is_open()){
            CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<"File open error: "<<file_;
            return false;
        }
        return true;
    }
    ...
    bool cReadImu::DecodeImu() {
        if(imu_data_.imu_type_==IMU_NOVTEL_CPT){
            ...
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Unsupport imu type";
            return false;
        }
        return true;
    }
    ...
    bool cReadImu::DecodeCsv(TIME_FORMAT time_f) {
        ...
        while(getline(inf_,line_str_)&&!inf_.eof()){
			...
			imu_data.odo.vr[0] = data[idx_odo_];
    ...
    bool cReadImu::DecodePos() {
        ...
        if(!in_file){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"OPEN IMU FILE ERROR: "<<file_;
            return false;
        }
        double data[7]={0};
        int n=0;
	...
	bool cReadImu::DecodeTXT(){
       // ifstream in_file(file_,ios::in | ios::binary);
        ifstream in_file(file_,ios::in);
        if(!in_file){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"OPEN IMU FILE ERROR: "<<file_;
            return false;
        }
        ...
        while(getline(in_file,temp)){
            ...
            imu_data.t_tag.t_.sec=data[0]-(int(data[0]));
          //  imu_data.t_tag+=gnss_week_*604800.0;
            imu_data.acce[0]=data[1];
            imu_data.acce[1]=data[2];
            imu_data.acce[2]=data[3];
            imu_data.gyro[0]=data[4];
            imu_data.gyro[1]=data[5];
            imu_data.gyro[2]=data[6];
            imu_data_.data_.push_back(imu_data);
            n++;
        }
        in_file.close();
    ...
    bool cReadImu::SetImu(tIGCOUPLEDConf C) {
        if(C.insC.imu_type==IMU_UNKNOW){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Unknow imu type";
            return false;
        }
    ...
    void cReadImu::SetImuTimeSpan(IGCoupled::cTime *ts, IGCoupled::cTime *te) {
        if(ts->TimeDiff(te->t_)>=0){
            CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<"Wrong time interval setting";
            return;
        }
        else{
    ...
    bool cReadRnx::ReadRnxHead() {
        ...
        if(rnx_type_.empty()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Rinex file type error";
            return false;
        }
        return true;
    }
    ...
    cReadGnssObs::cReadGnssObs(string file_path,IGCoupled::tNav& nav,cGnssObs& obss,RECEIVER_INDEX rcv) {
        file_=file_path;
        gnss_data_=&obss;
    ...
    bool cReadGnssObs::CmpEpochSatData(const IGCoupled::tSatObsUnit &p1, const IGCoupled::tSatObsUnit &p2) {
        return p1.sat.sat_.no<p2.sat.sat_.no;
    }

    bool cReadGnssObs::SortEpochSatData(IGCoupled::tEpochSatUnit& epoch_sat_data) {
    ...
    bool cReadGnssObs::DecodeEpochSatObs(tSatObsUnit& obs) {
        ...
        for(i=0,j=rnx_ver_<=2.99?0:3;i<gnss_signal->GetGnssSignal()->n&&j+15<line_str_.length();i++,j+=16){
            ...
            val[i]+=gnss_signal->GetGnssSignal()->shift[i];
             ...
             //原始观测值改正osb  zwl 2022.4.7
            if(val[i] != 0.0 && gnss_signal->GetGnssSignal()->type[i] == GNSS_OBS_CODE){
                val[i] -= nav_->code_osb[obs.sat.sat_.no-1][gnss_signal->GetGnssSignal()->code[i]];
            }else if(val[i] != 0.0 && gnss_signal->GetGnssSignal()->type[i] == GNSS_OBS_PHASE){
                val[i] -= nav_->phase_osb[obs.sat.sat_.no-1][gnss_signal->GetGnssSignal()->code[i]]*kGnssFreqs[obs.sat.sat_.sys_idx][gnss_signal->GetGnssSignal()->frq[i]-1]/CLIGHT;
            }
        ...
        while(getline(inf_,line_str_)&&!inf_.eof()){
            if (line_str_.find("MARKER NAME")!=string::npos && sta)
                ...
                if((i=kSatSysCode.find(line_str_[0]))==string::npos){
                    CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<"Invalid satellite system: "<<line_str_[0];
                    continue;
                }
                    ...
                    if(rnx_ver_<3.04){
                        for(j=0;j<num_signal;j++){
                            if(obs_type_code_[i][j][1]=='1'){
                                ...
                                CLOG(DEBUG,ELPP_CURR_FILE_LOGGER_ID)<<"BD2 change C1x to C2x";
                            }
                        }
                    }
                }
    ...
    bool cReadGnssObs::Reading() {
        ...
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open gnss obs file error: "<<file_;
            return false;
        }
    ...
    cReadGnssBrdEph::cReadGnssBrdEph(string file_path, IGCoupled::tNav &nav) {
        file_=file_path;
        nav_=&nav;
    }
    ...
    void cReadGnssBrdEph::DecodeEph(IGCoupled::cTime toc, IGCoupled::cSat sat, IGCoupled::tBrdEphUnit &brd_eph) {
        brd_eph.sat=sat;
        brd_eph.toc=toc;
    ...
    void cReadGnssBrdEph::DecodeGloEph(cTime toc,const cSat& sat,IGCoupled::tBrdGloEphUnit &glo_eph) {
        double tow,tod;
        int week,dow;
        cTime tof;
    ...
    bool cReadGnssBrdEph::Reading() {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open gnss broadcast ephemeris file error: "<<file_;
            return false;
        }
    ...
    cReadGnssPreEph::cReadGnssPreEph(string file_path, IGCoupled::tNav &nav) {
        file_=file_path;
        nav_=&nav;
        sys_mask_=SYS_ALL;
    }
    ...
    bool cReadGnssPreEph::Reading(int type) {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open gnss precise products file error: "<<file_;
            return false;
        }
    ...
    cReadGnssCodeBias::cReadGnssCodeBias(string file_path, IGCoupled::tNav &nav,int type) {
        file_=file_path;
        nav_=&nav;
        type_=type;
    }
    ...
    bool cReadGnssCodeBias::Reading() {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open gnss code bias file error: "<<file_;
            return false;
        }
        if(type_){
            DecodeCasMgexDcb();
    ...
    cReadGnssErp::cReadGnssErp(string file_path, IGCoupled::tNav &nav) {
        file_=file_path;
        nav_=&nav;
    }
    ...
    bool cReadGnssErp::Reading() {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open gnss erp file error: "<<file_;
            return false;
        }
    ...
    cReadGnssOcean::cReadGnssOcean(string file_path, IGCoupled::tNav &nav,string site,RECEIVER_INDEX idx) {
        file_=file_path;
        nav_=&nav;
        site_name_=site;
        index_=idx;
    ...
    bool cReadGnssOcean::Reading() {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open gnss ocean_load file error: "<<file_;
            return false;
        }
    ...
    cReadGnssAntex::cReadGnssAntex(string file_path, IGCoupled::tNav &nav) {
        file_=file_path;
    }
    ...
    void cReadGnssAntex::AlignAntPar2Sat(tIGCOUPLEDConf C,cTime t, tStaInfoUnit* sta,tAntUnit *sat_ant, tAntUnit *rec_ant) {
        int i;
        tAntUnit *ant;
        cSat sat;
    ...
    bool cReadGnssAntex::Reading() {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open gnss antex file error: "<<file_;
            return false;
        }
    ...
    cReadGnssIonex::cReadGnssIonex(string file_path,IGCoupled::tNav &nav) {
        file_=file_path;
        nav_=nav;
    }
    ...
    bool cReadGnssIonex::Reading() {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open gnss ionex file error: "<<file_;
            return false;
        }
    ...
    cReadFcb::cReadFcb(string path, IGCoupled::tNav &nav) {
        file_=path;
        nav_=&nav;
    }
    ...
    bool cReadFcb::Reading() {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open fcb file error: "<<file_;
            return false;
        }
        if(!ReadHead()) return false;
        DecodeFcb();
    ...
    cReadOsb::cReadOsb(string path, IGCoupled::tNav &nav) {
        file_=path;
        nav_=&nav;
    }
    ...
    void cReadOsb::DecodeOsb() {
        ...
        while(getline(inf_,line_str_)&&!inf_.eof()){
            if((!line_str_.compare(1,3,"OSB"))&&(!line_str_.compare(65,2,"ns"))){
                sat=cSat(line_str_.substr(11,3));
                sat.SatId2No();
                //type =line_str_.substr(25,3);
                Str2Double(line_str_.substr(70,21),bias);
                if(line_str_.substr(25,1) == "C"){
                    for(i=0; i<MAX_GNSS_CODE_TYPE;i++){
                        if(line_str_.substr(26,2)==kGnssSignalCodes[i]) break;
                    }
                    nav_->code_osb[sat.sat_.no-1][i] = bias * CLIGHT * 1E-9;
                }else if(line_str_.substr(25,1)=="L"){
                    for(i=0;i<MAX_GNSS_CODE_TYPE;i++){
                        if(line_str_.substr(26,2)==kGnssSignalCodes[i]) break;
                    }
                    nav_->phase_osb[sat.sat_.no-1][i]= bias * CLIGHT * 1E-9;
                }
    ...
    bool cReadOsb::Reading() {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open osb file error: "<<file_;
            return false;
        }
    ...
    bool cReadRefSol::Reading(int type) {
        if(!OpenFile()){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"Open reference solution file error: "<<file_;
            return false;
        }
    ...
    bool GetRefPosFrmSnx(tIGCOUPLEDConf& C,string path,Vector3d& ref_pos) {
        ifstream inf;
        int flag=0;
        ref_pos<<0,0,0;
        ...
        if(pos[0]!=0.0){
            ...
            CLOG(INFO,ELPP_CURR_FILE_LOGGER_ID)<<ref_coor;
            return true;
        }
        else{
            return false;
    ...
    bool ReadSol(tIGCOUPLEDConf& C,string path, vector<tSolInfoUnit>& gnss_sol) {
        ifstream inf;
        string buff;
