...
namespace IGCoupled{
    ...
    extern double *mat(int n, int m)
    {
        ...
        if (!(p=(double *)malloc(sizeof(double)*n*m))) {
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"matrix memory allocation error";
        }
        return p;
    }
    ...
    extern int *imat(int n, int m)
    {
        ...
        if (!(p=(int *)malloc(sizeof(int)*n*m))) {
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"integer matrix memory allocation error";
        }
        return p;
    }
...
/* LU back-substitution ------------------------------------------------------*/
    ...
    int cTime::Str2Time(string s) {
        if (sscanf(s.c_str(),"%lf %lf %lf %lf %lf %lf",epoch_,epoch_+1,epoch_+2,epoch_+3,epoch_+4,epoch_+5)<6){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"time string error";
            return -1;
        }

        if (epoch_[0]<100) epoch_[0]+=2000;
        if (epoch_[0]<=1900||epoch_[1]==0||epoch_[2]==0){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"time format error";
            return -1;
        }
    ...
    double cTime::TimeDiff(IGCoupled::tTime t1) {
...
//        if(dt> 43200.0) return *this+(-86400.0);
    ...
    cCoord::cCoord(const Vector3d& coord, const IGCoupled::COORDINATE_TYPE coord_type) {
    ...
    cCoord::cCoord(const double* coord, const IGCoupled::COORDINATE_TYPE coord_type) {
        if(coord_type==COORD_ENU){
    ...
    cParSetting::cParSetting(tIGCOUPLEDConf conf) {IGCOUPLEDC_=conf;}

    cParSetting::~cParSetting() {}

    int cParSetting::GetGnssUsedFrqs() {
        if(IGCOUPLEDC_.gnssC.ion_opt==ION_IF){
            if(IGCOUPLEDC_.gnssC.frq_opt==FRQ_SINGLE||IGCOUPLEDC_.gnssC.frq_opt==FRQ_DUAL) return 1;
            else if(IGCOUPLEDC_.gnssC.frq_opt==FRQ_TRIPLE) return 3;
        }
        else if(IGCOUPLEDC_.gnssC.ion_opt==ION_IF_DUAL) return 2;
        else return IGCOUPLEDC_.gnssC.frq_opt+1;
    }

    int cParSetting::GetNumObsType() {
        if(IGCOUPLEDC_.mode==MODE_SPP||IGCOUPLEDC_.mode_opt==MODE_OPT_SPP) return 1;
        else return 1+1;
    }

    int cParSetting::GetInsTransParNum(tIGCOUPLEDConf C) {
        IGCOUPLEDC_=C;
        return NumPos()+NumVel()+NumAtt()+NumBa()+NumBg()+NumSa()+NumSg()+NumRa()+NumRg()+NumLever();
    }

    int cParSetting::GetIGCOUPLEDPar(tIGCOUPLEDConf C) {
        IGCOUPLEDC_=C;
        int a=NumPos();
        int b=NumVel();
        int c=NumAtt();
    ...
    int cParSetting::GetRealFixParNum(tIGCOUPLEDConf C) {
        IGCOUPLEDC_=C;
        int a=NumPos();
        int b=NumVel();
        int c=NumAtt();
    ...
    int cParSetting::NumVel() {
        if(IGCOUPLEDC_.dynamic||IGCOUPLEDC_.mode>MODE_INS) return 3;
        else return 0;
    }

    int cParSetting::NumAtt() {
        if(IGCOUPLEDC_.mode>MODE_INS) return 3;
        else return 0;
    }
    ...
    int cParSetting::NumSa() {
        if(IGCOUPLEDC_.mode>MODE_INS&&IGCOUPLEDC_.insC.est_sa) return 3;
        else return 0;
    }

    int cParSetting::NumSg() {
        if(IGCOUPLEDC_.mode>MODE_INS&&IGCOUPLEDC_.insC.est_sg) return 3;
        else return 0;
    }

    int cParSetting::NumRa() {
        if(IGCOUPLEDC_.mode>MODE_INS&&IGCOUPLEDC_.insC.est_ra) return 6;
        else return 0;
    }

    int cParSetting::NumRg() {
        if(IGCOUPLEDC_.mode>MODE_INS&&IGCOUPLEDC_.insC.est_rg) return 6;
        else return 0;
    }

    int cParSetting::NumLever() {
        if(IGCOUPLEDC_.mode>MODE_INS&&IGCOUPLEDC_.insC.est_level) return 3;
        else return 0;
    }
    ...
    int cParSetting::NumClk() {
        int a=NSYS;
        if(IGCOUPLEDC_.mode==MODE_INS||IGCOUPLEDC_.mode==MODE_IGLC||IGCOUPLEDC_.mode==MODE_PPK||IGCOUPLEDC_.mode_opt==MODE_OPT_PPK) return 0;
        if(IGCOUPLEDC_.gnssC.est_bd3_isb) a+=1;
        return a;
    }

    int cParSetting::NumPhaseClk() {
        int a=NSYS;
        if((IGCOUPLEDC_.mode==MODE_PPP||IGCOUPLEDC_.mode_opt==MODE_OPT_PPP)&&IGCOUPLEDC_.gnssC.ar_mode==AR_PPP_AR&&IGCOUPLEDC_.gnssC.ar_prod==AR_PROD_IRC_CNES){
            return a;
        }
        else return 0;
    }
    ...
    int cParSetting::NumClkDrift() {
        if(IGCOUPLEDC_.mode==MODE_INS||IGCOUPLEDC_.mode==MODE_IGLC) return 0;
        if(IGCOUPLEDC_.gnssC.use_doppler) return NSYS;
    }

    int cParSetting::NumDcb() {
        if(IGCOUPLEDC_.mode==MODE_INS||IGCOUPLEDC_.mode==MODE_IGLC) return 0;
        if(IGCOUPLEDC_.gnssC.ion_opt==ION_CONST) return NSYS;
        return 0;
    }

    int cParSetting::NumIfb() {
        if(IGCOUPLEDC_.mode==MODE_INS||IGCOUPLEDC_.mode==MODE_IGLC||IGCOUPLEDC_.mode==MODE_PPK||IGCOUPLEDC_.mode_opt==MODE_OPT_PPK) return 0;
        if(IGCOUPLEDC_.gnssC.frq_opt>=FRQ_TRIPLE&&IGCOUPLEDC_.mode<MODE_INS) return NSYS;
        return 0;
    }

    int cParSetting::NumGloIfcb() {
        if(!(IGCOUPLEDC_.gnssC.nav_sys&SYS_GLO)) return 0;
        if(IGCOUPLEDC_.mode==MODE_PPK||IGCOUPLEDC_.mode_opt==MODE_OPT_PPK) return 0;

        if(IGCOUPLEDC_.mode==MODE_INS||IGCOUPLEDC_.mode==MODE_IGLC||IGCOUPLEDC_.mode==MODE_SPP||IGCOUPLEDC_.gnssC.glo_ifcb_opt==GLO_IFCB_OFF) return 0;
        if(IGCOUPLEDC_.gnssC.glo_ifcb_opt==GLO_IFCB_LNF) return 1;
        else if(IGCOUPLEDC_.gnssC.glo_ifcb_opt==GLO_IFCB_QUAD) return 2;
        else if(IGCOUPLEDC_.gnssC.glo_ifcb_opt==GLO_IFCB_1SAT) return NUM_GLO_SAT;
        else if(IGCOUPLEDC_.gnssC.glo_ifcb_opt==GLO_IFCB_1FRQ) return 13;
        else return 0;
    }
    ...
    int cParSetting::NumGloIfpb() {
        ...
        if((IGCOUPLEDC_.mode==MODE_PPK||IGCOUPLEDC_.mode_opt==MODE_OPT_PPK)&&IGCOUPLEDC_.gnssC.glo_ar_mode==GLO_AR_AUTO){
            return 2;
        }
        else return 0;
    }
    ...
    int cParSetting::NumTrp() {
        ...
        if(IGCOUPLEDC_.mode==MODE_INS||IGCOUPLEDC_.mode==MODE_IGLC) return 0;
        if(IGCOUPLEDC_.mode==MODE_PPK||IGCOUPLEDC_.mode_opt==MODE_OPT_PPK) ppk_mode=2;

        if(IGCOUPLEDC_.gnssC.trp_opt<=TRP_SAAS) return 0;
        else if(IGCOUPLEDC_.gnssC.trp_opt==TRP_EST_WET) return 1*ppk_mode;
        else if(IGCOUPLEDC_.gnssC.trp_opt==TRP_EST_GRAD) return (1+2)*ppk_mode;
    }

    int cParSetting::NumIon() {
        if(IGCOUPLEDC_.mode==MODE_INS||IGCOUPLEDC_.mode==MODE_IGLC) return 0;
        if(IGCOUPLEDC_.gnssC.ion_opt<=ION_IF_DUAL) return 0;
        else if(IGCOUPLEDC_.gnssC.ion_opt<=ION_CONST) return MAX_SAT_NUM;
    }

    int cParSetting::NumAmb() {
        if(IGCOUPLEDC_.mode==MODE_IGLC) return 0;
        if(IGCOUPLEDC_.mode==MODE_PPP||IGCOUPLEDC_.mode==MODE_PPK||IGCOUPLEDC_.mode_opt==MODE_OPT_PPP||IGCOUPLEDC_.mode_opt==MODE_OPT_PPK)
