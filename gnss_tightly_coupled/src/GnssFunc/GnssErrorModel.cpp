...
namespace IGCoupled{
    ...
    void cGnssModel::InitErrModel(tIGCOUPLEDConf C) {
        IGCOUPLEDC_=C;
    }
    ...
    Vector2d cTrpDelayModel::GetTrpError(double humi,double* x,int it) {
        ...
        if(IGCOUPLEDC_.gnssC.trp_opt==TRP_SAAS) GetSaasTrp(humi, nullptr,nullptr);
        else if(IGCOUPLEDC_.gnssC.trp_opt>=TRP_EST_WET){
            EstTrpWet(humi,x,it);
    ...
    Vector4d cTrpDelayModel::EstTrpWet(double humi,double *x,int it) {
        ...
        if(IGCOUPLEDC_.gnssC.trp_opt==TRP_EST_GRAD&&sat_info_->el_az[0]>0){
    ...
    Vector2d cIonDelayModel::GetIonError() {
        ...
        if(IGCOUPLEDC_.gnssC.ion_opt==ION_IF||IGCOUPLEDC_.gnssC.ion_opt==ION_IF_DUAL) IonFreeModel();
        else if(IGCOUPLEDC_.gnssC.ion_opt==ION_KLB) KlobModel();
        else if(IGCOUPLEDC_.gnssC.ion_opt>ION_EST){
            IonEstModel();
        }
        return ion_delay_;
    }
...
//        if(IGCOUPLEDC_.fileC.cbias.empty()){
//            TgdModel();
//            return;
//        }
        BsxModel();
    ...
    void cCbiasModel::InitCbiasModel(double cbias[MAX_SAT_NUM][MAX_GNSS_CODE_BIAS_PAIRS],vector<tBrdEphUnit>& brd_eph) {
        for(int i=0;i<MAX_SAT_NUM;i++){
            for(int j=0;j<MAX_GNSS_CODE_BIAS_PAIRS;j++){
				if(IGCOUPLEDC_.gnssC.ar_mode && (IGCOUPLEDC_.gnssC.ar_prod == AR_PROD_OSB_WUH || IGCOUPLEDC_.gnssC.ar_prod == AR_PROD_OSB_CNES)){
                    code_bias_[i][j] = 0.0; // OSB 不用加DCB
                }else code_bias_[i][j]=cbias[i][j];
			}
        }
        brd_eph_=brd_eph;
    }
    ...
    void cCbiasModel::UpdateSatInfo() {
        ...
        int *frqs=IGCOUPLEDC_.gnssC.gnss_frq[sys_idx];
        ...
        if(sys==SYS_BDS&&sat_info_->sat.sat_.prn>18){
            frqs=IGCOUPLEDC_.gnssC.gnss_frq[NSYS];
            bias=cbias_[NSYS];
        }

        sat_info_->code_bias[0]=bias[0];
        if(IGCOUPLEDC_.gnssC.frq_opt==FRQ_DUAL){
            sat_info_->code_bias[0]=bias[0];
            sat_info_->code_bias[1]=bias[1];
        }
        else if(IGCOUPLEDC_.gnssC.frq_opt==FRQ_TRIPLE){
            sat_info_->code_bias[0]=bias[0];
            sat_info_->code_bias[1]=bias[1];
    ...
    void cCbiasModel::BsxModel() {
        ...
        if(sys==SYS_GPS){
            ...
            if(IGCOUPLEDC_.gnssC.eph_opt==EPH_BRD){
                // base on b3
                if(sat_info_->sat.sat_.prn>18){
                    //BD3
            ...
            else if(IGCOUPLEDC_.gnssC.eph_opt==EPH_PRE){
                if(IGCOUPLEDC_.gnssC.ac_opt==AC_COM){
                    // base on b1b2
                    for(int i=0;i<MAX_GNSS_USED_FRQ_NUM;i++){
                        if(sat_info_->P_code[i]==GNSS_CODE_NONE){
                            cbias_[SYS_INDEX_BDS][i]=0.0;
                ...
                else if(IGCOUPLEDC_.gnssC.ac_opt==AC_WUM||IGCOUPLEDC_.gnssC.ac_opt==AC_GBM){
                    // base on b1b3
    ...
    void cAntModel::InitAntModel(tIGCOUPLEDConf C, tAntUnit *sat_ant, tAntUnit *rec_ant) {
        IGCOUPLEDC_=C;
        sat_ant_=sat_ant;
        rec_ant_=rec_ant;
    }
...
//                return false;
    ...
    bool cEphModel::PreEphModel(tSatInfoUnit *sat_info) {
        ...
        if(IGCOUPLEDC_.gnssC.ar_mode==AR_PPP_AR&&IGCOUPLEDC_.gnssC.ar_prod==AR_PROD_IRC_CNES){
        ...
        if(IGCOUPLEDC_.gnssC.ar_mode==AR_PPP_AR&&IGCOUPLEDC_.gnssC.ar_prod==AR_PROD_IRC_CNES&&sat_info->pre_pr_clk[0]!=0.0){
    ...
    void cTidModel::InitTidModel(tIGCOUPLEDConf C, vector<tErpUnit> erp, double ocean_par[2][66]) {
        IGCOUPLEDC_=C;
        erp_=erp;
        for(int i=0;i<2;i++){
            for(int j=0;j<66;j++){
    ...
    void cTidModel::TidCorr(cTime t, Vector3d rec_xyz, Vector3d& dr) {
        ...
        if(IGCOUPLEDC_.gnssC.tid_opt&TID_SOLID){
        ...
        else if(IGCOUPLEDC_.gnssC.tid_opt&TID_OCEAN){
            // 没有改海潮
        }
        else if(IGCOUPLEDC_.gnssC.tid_opt&TID_POLE){
            //没有改极潮
        }
    }
    ...
    void cGnssErrCorr::InitGnssErrCorr(tIGCOUPLEDConf C, tNav* nav) {
        eph_model_.IGCOUPLEDC_=cbia_model_.IGCOUPLEDC_=ion_model_.IGCOUPLEDC_=trp_model_.IGCOUPLEDC_=ant_model_.IGCOUPLEDC_=tid_model_.IGCOUPLEDC_=C;
    ...
    void cGnssErrCorr::PhaseWindUp(tSatInfoUnit &sat_info, Vector3d rec_xyz) {
        ...
        if(isnan(ph)) {
            ph=0.0;
            CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<sat_info.t_tag.GetTimeStr(1)<<" "<<sat_info.sat.sat_.id<<" phase wind up error";
        }
