// realization of function in GnssGunc.h
...
namespace IGCoupled{
    ...
    void cSat::SatPrn2No() {
        if(sat_.prn<=0){
            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"satellite prn error";
            return;
        }
...
//            CLOG(ERROR,ELPP_CURR_FILE_LOGGER_ID)<<"satellite no. error";
            return;
        }
    ...
    void cGnssObs::CarrierSmoothCode(int smooth_length) {
        ...
        for(int i=0;i<epoch_num;i++){                         /* i 历元 标号*/
            for(int j=0;j<obs_.at(i).sat_num;j++){            /* j 该历元的卫星（数量）标号 */
                sat_obs=&obs_.at(i).epoch_data.at(j);         /* i历元的j卫星的 观测值*/
                sat_no=sat_obs->sat.sat_.no;                  /* 该卫星的号码*/
                sys_idx=sat_obs->sat.sat_.sys_idx;            /* 该卫星的系统（GCER）*/
                for(int k=0;k<MAX_GNSS_FRQ_NUM;k++){          /* k 该卫星的频率号*/
	...
	void cGnssObs::ReverseObs(){
		std::reverse(obs_.begin(), obs_.end());
	}
    ...
    void cGnssObsOperator::ReAlignObs(tIGCOUPLEDConf C, tSatInfoUnit &sat_info, tSatObsUnit sat_obs, int f, int raw_idx,int frq_idx,int *glo_frq) {
    ...
    void cGnssObsOperator::MakeGnssObsComb(tIGCOUPLEDConf C, GNSS_OBS_COMB type, tSatInfoUnit* sat_info,const tSatInfoUnit previous_sat_info) {
        if(type==COMB_IF){
    ...
    double cGnssObsOperator::GnssObsLinearComb(tIGCOUPLEDConf  C,int i, int j, int k, tSatInfoUnit &sat_info, GNSS_OBS type,double* var) {
    ...
    void cGnssObsOperator::MwCycleSlip(tIGCOUPLEDConf C,double sample_dt,double dt,tSatInfoUnit* sat_info,tSatInfoUnit* base_sat,tTime last_time) {
        if(C.gnssC.frq_opt==FRQ_SINGLE) return;
        ...
        for(int f=0;f<num_loop;f++){
            ...
            if(fabs(dmw)>thres_mw){
                CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<sat_info->t_tag.GetTimeStr(1)<<" "<<sat_info->sat.sat_.id<<" "<<"MW DETECT CYCLE SLIP dmw=mw1-mw0 "<<dmw<<"="<<w1<<"-"<<w0<<" THRESHOLD="<<thres_mw ;
                if(f==0){
                    sat_info->slip[0]|=1;
                    sat_info->slip[1]|=1;
                }
    ...
    void cGnssObsOperator::GfCycleSlip(tIGCOUPLEDConf C,double sample_dt,double dt, tSatInfoUnit *sat_info, tSatInfoUnit* base_sat) {
        if(C.gnssC.frq_opt==FRQ_SINGLE) return;
        int del_ep;
        ...
        for(int f=0;f<num_loop;f++){
            ...
            if(fabs(dgf)>thres_gf){
                CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<sat_info->t_tag.GetTimeStr(1)<<" "<<sat_info->sat.sat_.id<<" "<<"GF DETECT CYCLE SLIP dgf=gf1-gf0 "<<dgf<<"="<<g1<<"-"<<g0<<" THRESHOLD="<<thres_gf;
                if(f==0) {
                    sat_info->slip[0]|=1;
                    sat_info->slip[1]|=1;
                 }
    ...
    bool cGnssObsOperator::LliCycleSlip(tIGCOUPLEDConf C, tSatInfoUnit& sat_info,int nf,double tt,RECEIVER_INDEX rcv,
                                        unsigned char *s) {
        int f;
        ...
        for(f=0;f<nf;f++){
            ...
            if(tt>=0.0){
                if(sat_info.LLI[f]&1){
                    CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<sat_info.t_tag.GetTimeStr(1)<<" "<<sat_info.sat.sat_.id<<" L"<<f+1<<" SLIP DETECTED FORWARD";
                }
                slip=sat_info.LLI[f];
            }
            else{
                if(LLI&1){
                    CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<sat_info.t_tag.GetTimeStr(1)<<" "<<sat_info.sat.sat_.id<<" L"<<f+1<<" SLIP DETECTED BACKWARD";
                }
                slip=LLI;
            }
    ...
    bool cGnssObsOperator::LliCycleSlip(tIGCOUPLEDConf C, tSatInfoUnit &sat_info, tSatInfoUnit &pre_sat_info, int nf, double tt, RECEIVER_INDEX rcv) {
        int f;
        unsigned int slip=0,LLI;
...
//            }
    ...
    bool cGnssObsOperator::CodeMinuPhase(tIGCOUPLEDConf C, tSatInfoUnit &sat_info,tSatInfoUnit &pre_sat_info) {
    ...
    void cGnssObsOperator::SmoothMw(tIGCOUPLEDConf C,tSatInfoUnit *sat_info,tSatInfoUnit *base_sat) {
    ...
    double GnssMeasVar(tIGCOUPLEDConf C,GNSS_OBS obs_type,tSatInfoUnit sat_info){
