...
namespace IGCoupled{
    /*
    * Function : The constructor and destructor function of class cDecodeGsof(Decode Gsof file)
    */
    cDecodeGsof::cDecodeGsof() {}

    cDecodeGsof::~cDecodeGsof() {}
    /*
    * Function : Decode Gsof file 
    * -Args : 
    *       string               file                  I              file name 
    *       vector<tSolInfoUnit> gsofs                 O              gsof information
    * -Returns : 
    ...
    */
    bool cDecodeGsof::DecodeGsof(const string file, vector<tSolInfoUnit>& gsofs) {
        ...
        if(!(fp=fopen(file.c_str(),"r"))){
            CLOG(WARNING,ELPP_CURR_FILE_LOGGER_ID)<<"FILE "<<file<<" OPEN ERROR";
            return false;
        }
        ...
        while(true) {
            ...
            if((InputGsofMessage(&raw,(unsigned char)data))){
                GsofUnit2igcoupledsol(raw.gsof_unit,sol);
                if(gsofs.size()>=3){
    ...
    bool cDecodeGsof::InputGsofMessage(IGCoupled::tRaw *raw, unsigned char data) {
        if(raw->dire) return DecodeMessageBackward(raw,data);
        else          return DecodeMessageForward(raw,data);
    }

    int cDecodeGsof::DecodeMessageForward(IGCoupled::tRaw *raw, unsigned char data) {
    ...
    int cDecodeGsof::DecodeMessageBackward(IGCoupled::tRaw *raw, unsigned char data) {

    }

    void cDecodeGsof::DecodeMessage(IGCoupled::tRaw *raw) {
    ...
    bool cDecodeGsof::CheckHead(IGCoupled::tRaw *raw) {
        return raw->buff[0]==0x02&&raw->buff[2]==0x40;
    }

    bool cDecodeGsof::CheckSum(IGCoupled::tRaw *raw) {
        unsigned char cs=0;
        int i;
    ...
    void cDecodeGsof::DecodeType01(IGCoupled::tRaw *raw, unsigned char *data) {
        int week;double sow;
    ...
    void cDecodeGsof::DecodeType02(IGCoupled::tRaw *raw, unsigned char *data) {
    ...
    void cDecodeGsof::DecodeType03(IGCoupled::tRaw *raw, unsigned char *data) {
    ...
    void cDecodeGsof::DecodeType06(IGCoupled::tRaw *raw, unsigned char *data) {
        raw->gsof_unit.rb_delta[0]=GetDouble(&data);
        raw->gsof_unit.rb_delta[1]=GetDouble(&data);
        raw->gsof_unit.rb_delta[2]=GetDouble(&data);
    }

    void cDecodeGsof::DecodeType08(IGCoupled::tRaw *raw, unsigned char *data) {
        double s,head;
    ...
    void cDecodeGsof::DecodeType09(IGCoupled::tRaw *raw, unsigned char *data) {
    ...
    void cDecodeGsof::DecodeType10(IGCoupled::tRaw *raw, unsigned char *data) {
//        raw->gsof_unit.clk.flags=data[0]; data++;
//        raw->gsof_unit.clk.off  =GetDouble(&data)/1000.0;
//        raw->gsof_unit.clk.foff =GetDouble(&data)/1E6;
    }

    void cDecodeGsof::DecodeType11(IGCoupled::tRaw *raw, unsigned char *data) {
    ...
    void cDecodeGsof::DecodeType12(IGCoupled::tRaw *raw, unsigned char *data) {
    ...
    void cDecodeGsof::DecodeType38(IGCoupled::tRaw *raw, unsigned char *data) {

    }

    void cDecodeGsof::DecodeType41(IGCoupled::tRaw *raw, unsigned char *data) {
...
//        raw->gsof_unit.base.quality=data[30];
    ...
    void cDecodeGsof::GsofUnit2igcoupledsol(IGCoupled::tGsofUnit &gsof, IGCoupled::tSolInfoUnit &igcoupled_sol) {
        Matrix3d Cen,covn,cove;
        Vector3d std,e;
        covn=Matrix3d::Zero();
        cove=Matrix3d::Zero();

        igcoupled_sol.t_tag=gsof.t_tag;

        igcoupled_sol.pos=gsof.pos;
        Cen=CalcCen(gsof.llh,COORD_ENU);
        igcoupled_sol.vel=Cen.transpose()*gsof.vel;
        ...
        for(int i=0;i<3;i++) igcoupled_sol.q_pos[i]=cove(i,i);
        for(int i=0;i<3;i++) igcoupled_sol.q_vel[i]=0.0;

        igcoupled_sol.valid_sat_num=gsof.num_use_sat;
        igcoupled_sol.stat= static_cast<SOL_STAT>(gsof.sol_level);
    }

    bool cDecodeGsof::CmpSolInfo(const IGCoupled::tSolInfoUnit &p1, const IGCoupled::tSolInfoUnit &p2) {
        cTime t1=p1.t_tag,t2=p2.t_tag;
        return t1.TimeDiff(t2.t_)<0.0;
    }
    /*
    * Function : The constructor and destructor of class cDecodeImuM39
    */
    cDecodeImuM39::cDecodeImuM39() {}

    cDecodeImuM39::~cDecodeImuM39() {}
    /*
    * Function : Decode M39-type IMU raw file
    * -Args : 
    *        string       file                 I            raw imu file
    *        tInscConf    insC                 I            ins data configure 
    *        tImuDataUnit imus                 IO           IMU data 
    * Returns:
    *        int          size                              The number of imu data         
    */
...
//        sort(imus.begin(),imus.end(),CmpImuData);
    ...
    int cDecodeImuM39::InputM39Message(IGCoupled::tRaw *raw, unsigned char data) {
        raw->len=NUMBYTES_GI310;
        if(raw->dire) return DecodeM39Backward(raw,data);
        else          return DecodeM39Forward(raw,data);
    }

    bool cDecodeImuM39::DecodeM39Forward(IGCoupled::tRaw *raw, unsigned char data) {
        raw->buff[raw->nbyte++]=data;
    ...
    bool cDecodeImuM39::DecodeM39Backward(IGCoupled::tRaw *raw, unsigned char data) {

    }

    int cDecodeImuM39::DecodeMessage(IGCoupled::tRaw *raw) {
        int i,week=0;
        unsigned int dc=0;
        static int start=0;
    ...
    void cDecodeImuM39::DecodeSowTime(IGCoupled::tRaw *raw, double *sow, int *start) {
    ...
    void cDecodeImuM39::AdjustTime(IGCoupled::tRaw *raw, const double sowi, double *sowo, double *timu, int *week,
                                   unsigned int *dcc) {
    ...
    bool cDecodeImuM39::DecodeM39Odo(IGCoupled::tRaw *raw, double dt) {
        static int dc;
